...
public class ExampleInstrumentedTest {
    ...
    @Test
    public void testTestPublish() throws JSONException {
        if (!BuildConfig.DEBUG) {
            throw new RuntimeException("test version must is debug!");
        }
        String pathname = "../apk";
        ...
        File apk = null;
        String testName = BuildConfig.VERSION_NAME + "_" + BuildConfig.VERSION_CODE + "_coexistCustomtest.apk";
        for (File f : files) {
...
//        https://nirvana1234.xyz/v2/api/android/apk/upload?requestId=1749357139371102208

        HttpResult<String> result = HttpClient.getApi(UploadServices.class, "https://api-gateway.cjm3.kf315.net/", client)
                .getUploadInfo(map)
                .blockingSingle();
        ...
        testPublish(fullPath);
    }

    public static void testPublish(String fullPath) {
        ...
        map.put("upgradeUrl", "http://filetest.jgwcjm.com/" + fullPath);
        HttpResult<String> result = HttpClient.getApi(UploadServices.class, "https://api-gateway.cjm3.kf315.net/", new OkHttpClient().newBuilder()
                        .build())
                .postPublish(map)
                .blockingSingle();
