package com.ggb.nirvanahappyclub.module.community.android.adapter
...
class CommunityAndroidAdapter : CustomRecyclerAdapter<CommunityAndroidListBean>() {
        ...
        return if (viewType == ITEM_TYPE_CONTENT1) {
            ContentType1ViewHolder(
                DataBindingUtil.inflate(
                    mLayoutInflater, R.layout.item_community_android, parent, false
                )
            )
        } else null
    }
    ...
    inner class ContentType1ViewHolder(binding: ItemCommunityAndroidBinding) : ContentViewHolder<ItemCommunityAndroidBinding>(binding) {
        init {
