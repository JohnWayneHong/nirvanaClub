 ...
 * @Description 养殖离场 Activity
 */
public class BreedOutDetailsActivity extends BaseScanActivity<BreedOutDetailsViewModel, ActivityCommonBreedDetailsBinding> {
    private int lastSelectTab = -1;
    private ArrayList<String> tags;
    private ArrayList<Fragment> fragments;
    private BreedOutDetailsFragment mBreedOutDetailsFragment;
    private BreedOutDetailsEarFragment mBreedOutDetailsEarFragment;

    @Override
    protected void initView() {
        mBindingView.tvBreedDetailsList.setText(getResources().getString(R.string.breed_out_batch_details));
    }
    ...
    protected void initData() {
        ...
        tags.add("mBreedOutDetailsEarFragment");

        fragments = new ArrayList<>();
        if (mBreedOutDetailsFragment == null) {
            BreedOutDetailsFragment fragmentByTag = (BreedOutDetailsFragment) fm.findFragmentByTag(tags.get(0));
            if (fragmentByTag == null) {
                mBreedOutDetailsFragment = new BreedOutDetailsFragment();
            } else {
                mBreedOutDetailsFragment = fragmentByTag;
            }
        }
        fragments.add(mBreedOutDetailsFragment);

        if (mBreedOutDetailsEarFragment == null) {
            BreedOutDetailsEarFragment fragmentByTag = (BreedOutDetailsEarFragment) fm.findFragmentByTag(tags.get(1));
            if (fragmentByTag == null) {
                mBreedOutDetailsEarFragment = new BreedOutDetailsEarFragment();
            } else {
                mBreedOutDetailsEarFragment = fragmentByTag;
            }
        }
        fragments.add(mBreedOutDetailsEarFragment);
    ...
    public static void start(Activity context, BreedOutDetailsBean data) {
        if (isActivityNotFinished(context)) {
            MMKVUtils.saveTempData(data);
            Intent intent = new Intent(context, BreedOutDetailsActivity.class);
