...
import com.jgw.delingha.custom_module.delingha.slaughter_weighing.details.SlaughterWeighingDetailsActivity;
 ...
 * 屠宰管理 宰后称重 列表
 */
public class SlaughterWeighingListActivity extends BaseActivity<SlaughterWeighingListViewModel, ActivityCommonSelectListButtonBinding> implements SwipeRefreshLayout.OnRefreshListener {

    private EditText searchView;
    private SlaughterWeighingListRecyclerAdapter mAdapter;
    ...
    protected void initView() {
        ...
        mBindingView.tvSelectCommonBottomButton.setText("新增称重记录");
    }

    @Override
    protected void initData() {
        setTitle("宰后称重记录");
        mAdapter = new SlaughterWeighingListRecyclerAdapter();
    ...
    public void initLiveData() {
        super.initLiveData();
        mViewModel.getSlaughterWeighingListLiveData().observe(this, resource -> {
        ...
        mViewModel.getSlaughterWeighingDetailsLiveData().observe(this, resource -> {
            switch (resource.getLoadingStatus()) {
                case Resource.LOADING:
                    showLoadingDialog();
                    break;
                case Resource.SUCCESS:
                    ...
                    SlaughterWeighingDetailsActivity.start(this, resource.getData());
                    break;
                case Resource.ERROR:
                    dismissLoadingDialog();
                    break;
        ...
        mViewModel.getDeleteSlaughterWeighingLiveData().observe(this, resource -> {
            switch (resource.getLoadingStatus()) {
                case Resource.LOADING:
                    showLoadingDialog();
                    break;
    ...
    public void onClick(View view) {
        ...
        if (id == mBindingView.tvSelectCommonBottomButton.getId()) {
            SlaughterWeighingDetailsActivity.start(this, 1);
        }
    }
    ...
    public void onItemClick(View view, int position) {
        ...
        if (id == R.id.tv_slaughter_weighing_list_details) {
            mViewModel.getSlaughterWeighingDetails(id1);
        } else if (id == R.id.tv_slaughter_weighing_list_delete) {
            CommonDialogUtil.showSelectDialog(this, "提示", "确认是否删除?", "取消", "确认", new CommonDialog.OnButtonClickListener() {
                @Override
                public void onRightClick() {
                    mViewModel.deleteSlaughterWeighing(id1);
    ...
    public static void start(Context context) {
        if (BaseActivity.isActivityNotFinished(context)) {
            Intent intent = new Intent(context, SlaughterWeighingListActivity.class);
