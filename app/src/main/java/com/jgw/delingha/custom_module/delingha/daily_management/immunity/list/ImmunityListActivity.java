...
import com.jgw.delingha.custom_module.delingha.daily_management.immunity.details.ImmunityDetailsActivity;
 ...
 * @author : hwj
 * @date : 2023/8/4
 * description : 免疫记录 列表 Activity
 */

public class ImmunityListActivity extends BaseActivity<ImmunityListViewModel, ActivityCommonSelectListButtonBinding> implements SwipeRefreshLayout.OnRefreshListener {

    private EditText searchView;
    private ImmunityListRecyclerAdapter mAdapter;
    ...
    protected void initData() {
        setTitle("免疫记录");
        mAdapter = new ImmunityListRecyclerAdapter();
    ...
    public void initLiveData() {
        ...
        mViewModel.getDeleteImmunityLiveData().observe(this, resource -> {
            switch (resource.getLoadingStatus()) {
                case Resource.LOADING:
                    showLoadingDialog();
                    break;
    ...
    public void onClick(View view) {
        ...
        if (id == mBindingView.tvSelectCommonBottomButton.getId()) {
            ImmunityDetailsActivity.start(this, 1);
        }
    }
    ...
    public static void start(Context context) {
        if (BaseActivity.isActivityNotFinished(context)) {
            Intent intent = new Intent(context, ImmunityListActivity.class);
