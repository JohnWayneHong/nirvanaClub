...
import com.jgw.delingha.custom_module.delingha.daily_management.health_care.details.HealthCareDetailsActivity;
 ...
 * @author : hwj
 * @date : 2023/8/4
 * description : 保健记录 列表 Activity
 */

public class HealthCareListActivity extends BaseActivity<HealthCareListViewModel, ActivityCommonSelectListButtonBinding> implements SwipeRefreshLayout.OnRefreshListener {

    private EditText searchView;
    private HealthCareListRecyclerAdapter mAdapter;

    @Override
    protected void initView() {
        searchView = getSearchView();
        if (searchView != null) {
            searchView.setHint("请输入养殖栏模糊搜索");
        }
    ...
    protected void initData() {
        setTitle("保健记录");
        mAdapter = new HealthCareListRecyclerAdapter();
    ...
    public void initLiveData() {
        ...
        mViewModel.getDeleteHealthCareLiveData().observe(this, resource -> {
            switch (resource.getLoadingStatus()) {
                case Resource.LOADING:
                    showLoadingDialog();
                    break;
    ...
    public void onClick(View view) {
        ...
        if (id == mBindingView.tvSelectCommonBottomButton.getId()) {
            HealthCareDetailsActivity.start(this, 1);
        }
    }
    ...
    public static void start(Context context) {
        if (BaseActivity.isActivityNotFinished(context)) {
            Intent intent = new Intent(context, HealthCareListActivity.class);
