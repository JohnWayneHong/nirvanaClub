 ...
 * @Description 养殖进场 关联耳号 Fragment
 */
public class BreedInDetailsEarFragment extends BaseFragment<BreedInDetailsViewModel, ActivityCommonBreedDetailsEarBinding> implements SwipeRefreshLayout.OnRefreshListener {

    public BreedInAssociateEarAdapter mAdapter;

    private BreedInDetailsBean tempData;
    ...
    protected void initFragmentData() {
        ...
        tempData = MMKVUtils.getTempData(BreedInDetailsBean.class);
        ...
        if (view.getId() == mBindingView.layoutBreedActionFooter.tvScanBack.getId()) {
            ...
            CommonDialogUtil.showInputDialog(context, "耳号", "请输入", "取消", "确定", new CommonDialog.OnButtonClickListener() {
                ...
                public boolean onInput(String input) {
                    ((BreedInDetailsActivity) Objects.requireNonNull(getActivity())).scanable = true;
    ...
    private void jumpScanBack() {
        ...
        BreedInScanBackActivity.start(this,  1,tempData.breedInBatch);

    }

    @Subscribe
    public void onScanQRCode(CommonEvent.ScanRFIDEvent event) {
        if (getActivity() instanceof BreedInDetailsActivity) {
            if (((BreedInDetailsActivity) getActivity() ).scanable){
                ((BreedInDetailsActivity) getActivity() ).scanable = false;
