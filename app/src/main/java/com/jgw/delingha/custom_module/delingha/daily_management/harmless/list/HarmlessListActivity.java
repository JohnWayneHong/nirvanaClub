...
import com.jgw.delingha.custom_module.delingha.daily_management.harmless.details.HarmlessDetailsActivity;
 ...
 * @author : hwj
 * @date : 2023/8/4
 * description : 无害化记录 列表 Activity
 */

public class HarmlessListActivity extends BaseActivity<HarmlessListViewModel, ActivityCommonSelectListButtonBinding> implements SwipeRefreshLayout.OnRefreshListener {

    private EditText searchView;
    private HarmlessListRecyclerAdapter mAdapter;
    ...
    protected void initData() {
        setTitle("无害化记录");
        mAdapter = new HarmlessListRecyclerAdapter();
    ...
    public void initLiveData() {
        ...
        mViewModel.getDeleteHarmlessLiveData().observe(this, resource -> {
            switch (resource.getLoadingStatus()) {
                case Resource.LOADING:
                    showLoadingDialog();
                    break;
    ...
    public void onClick(View view) {
        ...
        if (id == mBindingView.tvSelectCommonBottomButton.getId()) {
            HarmlessDetailsActivity.start(this, 1);
        }
    }
    ...
    public static void start(Context context) {
        if (BaseActivity.isActivityNotFinished(context)) {
            Intent intent = new Intent(context, HarmlessListActivity.class);
