...
import com.jgw.delingha.custom_module.maogeping.packaging_association_custom.viewmodel.PackagingAssociationCustomViewModel;
import com.jgw.delingha.databinding.ActivityPackagingAssociationCustomBinding;
 ...
 * author : xsw
 * data : 2020/12/1
 * description : 包装关联(毛戈平定制版)
 */
public class PackagingAssociationCustomActivity extends BaseActivity<PackagingAssociationCustomViewModel, ActivityPackagingAssociationCustomBinding> {

    private static final int TYPE_SCAN_BACK = 101;

    private CodeEntityRecyclerAdapter<PackagingAssociationCustomEntity> mAdapter;
    ...
    protected void initView() {
        setTitle("包装关联");
        setRight("输入");
        mBindingView.rvcPackaging.setEmptyLayout(R.layout.item_empty);
    }
    ...
    public void onScanQRCode(CommonEvent.ScanQRCodeEvent event) {
        ...
        if (CheckCodeUtils.checkCode(code)) {//检查码是否符合规范
            ...
            if (mViewModel.isRepeatCode(code)) {
                //检查码是否重复
                //检查重复的码是否为箱码
                return;
            }
    ...
    public void initLiveData() {
        ...
        mViewModel.getCheckCodeLiveData().observe(this, resource -> {
            PackagingAssociationCustomEntity entity = resource.getData();
    ...
    private void updateShowBoxList(Resource<PackagingAssociationCustomEntity> resource) {
        PackagingAssociationCustomEntity entity = resource.getData();
        ...
        List<PackagingAssociationCustomEntity> sonList = entity.getSonList();
        mAdapter.notifyRemoveListItem();
        if (sonList != null) {
    ...
    private void updateBoxCodeStatus(String code, int status) {
        ...
        if (boxCodeItemVisible == View.GONE) {
            mBindingView.layoutPackaingAssociationHeader.flItemPda.setVisibility(View.GONE);
        } else {
            mBindingView.layoutPackaingAssociationHeader.flItemPda.setVisibility(View.VISIBLE);
            mBindingView.layoutPackaingAssociationHeader.layoutBoxCode.setData(entity);

            mBindingView.layoutPackaingAssociationHeader.layoutBoxCode.tvChecking.setVisibility(isSuccess ? View.GONE : View.VISIBLE);
            mBindingView.layoutPackaingAssociationHeader.layoutBoxCode.ivCheckOk.setVisibility(isSuccess ? View.VISIBLE : View.GONE);
        }
    ...
    private void updateCodeStatus(String code, int status) {
        PackagingAssociationCustomEntity tempEntity = new PackagingAssociationCustomEntity();
        tempEntity.setOuterCode(code);
        int index = mAdapter.getDataList().indexOf(tempEntity);
        if (index == -1) {
            return;
        }
        PackagingAssociationCustomEntity entity = mAdapter.getDataList().get(index);
        switch (status) {
            case Resource.SUCCESS:
                entity.setCodeStatus(CodeBean.STATUS_CODE_SUCCESS);
    ...
    private void setConfigInfo(PackageConfigEntity entity) {
        mBindingView.layoutPackaingAssociationHeader.setData(entity);
        boolean empty = TextUtils.isEmpty(entity.getProductBatchName());
        mBindingView.layoutPackaingAssociationHeader.llProductBatch.setVisibility(empty ? View.GONE : View.VISIBLE);
    }
            ...
            CommonDialogUtil.showSelectDialog(this, "是否退出当前包装关联操作?", "若退出,您所操作的数据将被放入待执行.",
                    "继续退出", "立即上传", new CommonDialog.OnButtonClickListener() {
                        ...
                        public void onLeftClick() {
                            MainActivity.start(PackagingAssociationCustomActivity.this, 0);
                        }
    ...
    private void showUploadFinishDialog(UploadResultBean data) {
        ...
        CommonDialogUtil.showSelectDialog(this, "数据上传成功", details, "返回工作台", "查看状态", new CommonDialog.OnButtonClickListener() {
            ...
            public void onRightClick() {
                MainActivity.start(PackagingAssociationCustomActivity.this, 0);
                TaskListActivity.start(TaskListViewModel.TYPE_TASK_PACKAGING_ASSOCIATION, MainActivity.mMainActivityContext);
            }
        });
    }
    ...
    public static void start(Context context, long configId) {
        if (isActivityNotFinished(context)) {
            Intent intent = new Intent(context, PackagingAssociationCustomActivity.class);
