 ...
 */
public class FeedingDetailsViewModel extends BaseViewModel {
    private final FeedingDetailsModel model;
    ...
    public FeedingDetailsViewModel(@NonNull Application application) {
        super(application);
        model = new FeedingDetailsModel();
    }
...
//    }
    ...
    public void submit(InfoDetailsRecyclerAdapter adapter) {
        ...
        String startTime = adapter.searchValueStrByKey("饲喂开始");
        String endTime = adapter.searchValueStrByKey("饲喂结束");
        if (!TextUtils.isEmpty(startTime) && !TextUtils.isEmpty(endTime)) {
            ...
            if (startDate != null && endDate != null) {
                if (startDate.getTime() > endDate.getTime()) {
                    ToastUtils.showToast("饲喂开始时间不得大于饲喂结束时间!");
                    return;
                }
            }
        }
        map.put("feedStartTime", startTime);
        map.put("feedEndTime", endTime);


        map.put("totalFeed", adapter.searchValueStrByKey("饲料总重"));

        map.put("totalFeedAmount", adapter.searchValueStrByKey("饲料总价"));
