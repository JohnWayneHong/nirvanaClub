...
public class BreedOutBatchListActivity extends BaseSelectItemListActivity<BreedOutBatchListViewModel, ActivityCommonSelectListBinding> {
    public static final String TAG = "BreedOutBatchListActivity";
    public static final String TAG_DATA = "BreedOutBatchListActivityData";
    ...
    protected void onSelectItem(View view, int position) {
        ...
        if (!TextUtils.isEmpty(data) && (Integer.parseInt(data) <= 0)) {
            ToastUtils.showToast("批次"+selectItemSupport.getShowName()+"无可用离场数量");
            return;
        }
    ...
    public static void start(Activity context, int requestCode, String titleName) {
        if (isActivityNotFinished(context)) {
            Intent intent = new Intent(context, BreedOutBatchListActivity.class);
