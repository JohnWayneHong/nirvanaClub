...
import com.jgw.delingha.custom_module.delingha.daily_management.treatment.details.TreatmentDetailsActivity;
 ...
 * @author : hwj
 * @date : 2023/8/4
 * description : 诊疗记录 列表 Activity
 */

public class TreatmentListActivity extends BaseActivity<TreatmentListViewModel, ActivityCommonSelectListButtonBinding> implements SwipeRefreshLayout.OnRefreshListener {

    private EditText searchView;
    private TreatmentListRecyclerAdapter mAdapter;
    ...
    protected void initData() {
        setTitle("诊疗记录");
        mAdapter = new TreatmentListRecyclerAdapter();
    ...
    public void initLiveData() {
        ...
        mViewModel.getDeleteTreatmentLiveData().observe(this, resource -> {
            switch (resource.getLoadingStatus()) {
                case Resource.LOADING:
                    showLoadingDialog();
                    break;
    ...
    public void onClick(View view) {
        ...
        if (id == mBindingView.tvSelectCommonBottomButton.getId()) {
            TreatmentDetailsActivity.start(this, 1);
        }
    }
    ...
    public static void start(Context context) {
        if (BaseActivity.isActivityNotFinished(context)) {
            Intent intent = new Intent(context, TreatmentListActivity.class);
