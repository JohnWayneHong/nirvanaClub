 ...
 * 养殖进场 耳号反扫
 */
public class BreedInScanBackActivity extends BaseScanActivity<BreedInDetailsViewModel, ActivityBreedScanBackBinding> {

    private BreedInScanBackRecyclerAdapter mAdapter;
    ...
    protected void initData() {
        ...
        mAdapter = new BreedInScanBackRecyclerAdapter();
        mBindingView.rvCommonScanBack.setAdapter(mAdapter);
        mViewModel.setBatchIn(getIntent().getStringExtra("batchIn"));
        calculationTotal();
    }
    ...
    public static void start(Fragment fragment,int requestCode,String batchIn) {
        if (fragment == null || fragment.getActivity() == null) {
            return;
        }
        FragmentActivity context = fragment.getActivity();
        Intent intent = new Intent(context, BreedInScanBackActivity.class);
        intent.putExtra("batchIn",batchIn);
