...
import com.jgw.delingha.module.exchange_warehouse.adapter.ExchangeWarehousePendingConfigurationAdapter;
import com.jgw.delingha.module.exchange_warehouse.viewmodel.ExchangeWarehouseWaitUploadViewModel;
 ...
 * author : Cxz
 * data : 2020/3/4
 * description :  调仓执行配置列表界面
 */
public class ExchangeWarehouseWaitUploadActivity extends BaseActivity<ExchangeWarehouseWaitUploadViewModel, ActivityWaitUploadListBinding> implements OnItemSingleClickListener {

    private ExchangeWarehousePendingConfigurationAdapter mAdapter;
    ...
    protected void initData() {
        mAdapter = new ExchangeWarehousePendingConfigurationAdapter();
    ...
    private void showUploadFinishDialog(UploadResultBean data) {
        ...
        CommonDialogUtil.showSelectDialog(this,"数据上传成功", details, "返回工作台", "查看状态", new CommonDialog.OnButtonClickListener() {
            ...
            public void onLeftClick() {
                MainActivity.start(ExchangeWarehouseWaitUploadActivity.this, 0);
            }

            @Override
            public void onRightClick() {
                MainActivity.start(ExchangeWarehouseWaitUploadActivity.this, 0);
                TaskListActivity.start(TaskListViewModel.TYPE_TASK_EXCHANGE_WAREHOUSE, ExchangeWarehouseWaitUploadActivity.this);
            }
        });
    }
    ...
    public void onItemClick(View view, int position) {
        ExchangeWarehouseConfigurationPendingBean bean = mAdapter.getDataList().get(position);
        int id = view.getId();
        if (id == R.id.iv_select) {
            bean.isSelect = !bean.isSelect;
            mAdapter.notifyRefreshItem(bean);
        } else if (id == R.id.fl_configuration) {
            CommonScanBackActivity.start(this, 1, CommonScanBackActivity.TYPE_EXCHANGE_WAREHOUSE, bean.id);
        }
    }
    ...
    public static void start(Context context) {
        if (isActivityNotFinished(context)) {
            Intent intent = new Intent(context, ExchangeWarehouseWaitUploadActivity.class);
