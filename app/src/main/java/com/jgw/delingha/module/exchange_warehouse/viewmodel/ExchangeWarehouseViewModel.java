 ...
 * author : Cxz
 * data : 2020/2/26
 * description : 调仓设置ViewModel
 */
public class ExchangeWarehouseViewModel extends BaseViewModel {

    private final static int CAPACITY = CustomRecyclerAdapter.ITEM_PAGE_SIZE;//展示list最大展示量
    private int mPage = 1;
    private List<ExchangeWarehouseEntity> showList = new ArrayList<>();//展示list
    private final ExchangeWarehouseModel mModel;
    private ExchangeWarehouseConfigurationEntity mConfigEntity;//配置Entity类
    ...
    public ExchangeWarehouseViewModel(@NonNull Application application) {
        super(application);
        mModel = new ExchangeWarehouseModel();
    }

    public void setDataList(List<ExchangeWarehouseEntity> list) {
        showList = list;
    }
    ...
    public LiveData<Resource<ExchangeWarehouseConfigurationEntity>> getConfigurationEntityLiveData() {
        return Transformations.switchMap(mConfigLiveData, mModel::getConfig);
    }

    public void setConfigEntity(ExchangeWarehouseConfigurationEntity entity) {
        mConfigEntity = entity;
    }

    public void getCodeCount() {
        mGetCountLiveData.setValue(mConfigEntity.getId());
    }

    public LiveData<Resource<Map<String, Long>>> getCountLiveData() {
        return Transformations.switchMap(mGetCountLiveData, mModel::getCount);
    }

    public void handleScanQRCode(String code, CodeEntityRecyclerAdapter<ExchangeWarehouseEntity> adapter, CustomBaseRecyclerView recyclerView) {
        if (!CheckCodeUtils.isV3(code)) {
            ToastUtils.showToast(code + ResourcesUtils.getString(R.string.pda_scan_error_code_toast));
            return;
        }
        ExchangeWarehouseEntity data = mModel.getExchangeWarehouseEntity(code);
        if (data != null) {
        ...
        ExchangeWarehouseEntity entity = new ExchangeWarehouseEntity();
        ...
        mPage = 1;
        getCodeCount();
        checkCodeParamsBean bean = new checkCodeParamsBean(3, code, mConfigEntity.getOutWareHouseId(), true);
     ...
     * 更改检查接口通过后Code的信息
     */
    public void updateCode(CodeBean code, CodeEntityRecyclerAdapter<ExchangeWarehouseEntity> adapter) {
        for (int i = 0; i < showList.size(); i++) {
            ExchangeWarehouseEntity entity = showList.get(i);
            if (TextUtils.equals(entity.getOuterCode(), code.outerCodeId)) {
                entity.setCodeStatus(CodeBean.STATUS_CODE_SUCCESS);
                entity.setSingleNumber(code.singleNumber);
    ...
    public void updateErrorCode(String code, CodeEntityRecyclerAdapter<ExchangeWarehouseEntity> adapter) {
        mModel.updateCodeStatusByCode(code, CodeBean.STATUS_CODE_FAIL);
        for (int i = 0; i < showList.size(); i++) {
            if (TextUtils.equals(showList.get(i).getOuterCode(), code)) {
                showList.get(i).setCodeStatus(CodeBean.STATUS_CODE_FAIL);
                adapter.notifyItemChanged(i);
                break;
            }
        }
    ...
    public void deleteCode(String code, CodeEntityRecyclerAdapter<ExchangeWarehouseEntity> mAdapter) {
        ExchangeWarehouseEntity exchangeGoodsEntity = new ExchangeWarehouseEntity();
     ...
     * 获取反扫回来的更新
     */
    public LiveData<Resource<List<ExchangeWarehouseEntity>>> getRefreshListDataLiveData() {
    ...
    public LiveData<Resource<List<ExchangeWarehouseEntity>>> getLoadMoreLiveData() {
