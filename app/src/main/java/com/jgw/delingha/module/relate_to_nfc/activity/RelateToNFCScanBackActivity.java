...
import com.jgw.delingha.module.relate_to_nfc.adapter.RelateToNFCScanBackAdapter;
import com.jgw.delingha.module.relate_to_nfc.viewmodel.RelateToNFCScanBackViewModel;
...
public class RelateToNFCScanBackActivity extends BaseActivity<RelateToNFCScanBackViewModel, ActivityCommonScanBackBinding> {

    private RelateToNFCScanBackAdapter mAdapter;
    ...
    protected void initData() {
        mAdapter = new RelateToNFCScanBackAdapter();
    ...
    public void initLiveData() {
            ...
            switch (resource.getLoadingStatus()) {
                case Resource.SUCCESS:
                    if (mViewModel.getCurrentPage() == 1) {
                        List<RelateToNFCEntity> data = resource.getData();
        ...
        mViewModel.getDeleteCodeByQRCodeLiveData().observe(this, resource -> {
            switch (resource.getLoadingStatus()) {
                case Resource.SUCCESS:
        ...
        mViewModel.getDeleteCodeByNFCCodeLiveData().observe(this, resource -> {
            switch (resource.getLoadingStatus()) {
                case Resource.SUCCESS:
                    mAdapter.notifyRemoveItem(resource.getData()[0]);
                    if (resource.getData()[1] != null) {
    ...
    public void onItemClick(View view, int position) {
        String qrCode = mAdapter.getDataList().get(position).getQRCode();
        mViewModel.handleScanQRCode(qrCode);
    }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        for (String s : strings) {
            mViewModel.handleScanNFCCode(s);
        }
    }
    ...
    public static void start(Activity context, int requestCode) {
        if (isActivityNotFinished(context)) {
            Intent intent = new Intent(context, RelateToNFCScanBackActivity.class);
