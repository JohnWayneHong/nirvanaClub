 ...
 * description : 通用包装反扫类
 */
public class CommonPackageScanBackAdapter extends CustomRecyclerAdapter<BasePackageCodeEntity> {

    @Override
    public ContentViewHolder<? extends ViewDataBinding> onCreateCustomViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == ITEM_TYPE_CONTENT1) {
            return new CommonPackageScanBackAdapter.ContentType1ViewHolder(DataBindingUtil.inflate(mLayoutInflater
    ...
    public void onBindCustomViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof CommonPackageScanBackAdapter.ContentType1ViewHolder) {
            BaseCodeEntity entity = mList.get(position);
            ((CommonPackageScanBackAdapter.ContentType1ViewHolder) holder).mBindingView.setData(entity);
        }
    }
