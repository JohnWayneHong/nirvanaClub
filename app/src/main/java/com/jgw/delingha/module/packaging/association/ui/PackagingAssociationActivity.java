...
import com.jgw.delingha.databinding.ActivityPackagingAssociationBinding;
import com.jgw.delingha.module.main.MainActivity;
import com.jgw.delingha.module.packaging.association.viewmodel.PackagingAssociationViewModel;
 ...
 * description : 包装关联
 */
public class PackagingAssociationActivity extends BaseActivity<PackagingAssociationViewModel, ActivityPackagingAssociationBinding> {

    private static final int TYPE_SCAN_BACK = 101;

    private CodeEntityRecyclerAdapter<PackagingAssociationEntity> mAdapter;
    ...
    public void initLiveData() {
        ...
        mViewModel.getCheckCodeLiveData().observe(this, resource -> {
            PackagingAssociationEntity entity = resource.getData();
    ...
    private void updateShowBoxList(Resource<PackagingAssociationEntity> resource) {
        PackagingAssociationEntity entity = resource.getData();
        ...
        List<PackagingAssociationEntity> sonList = entity.getSonList();
        mAdapter.notifyRemoveListItem();
        if (sonList != null) {
    ...
    private void updateBoxCodeStatus(String code, int status) {
        int boxCodeItemVisible = View.VISIBLE;
        ...
        mBindingView.layoutPackaingAssociationHeader.flItemPda.setVisibility(boxCodeItemVisible);
    ...
    private void updateCodeStatus(String code, int status) {
        PackagingAssociationEntity tempEntity = new PackagingAssociationEntity();
        tempEntity.setOuterCode(code);
        int index = mAdapter.getDataList().indexOf(tempEntity);
        if (index == -1) {
            return;
        }
        PackagingAssociationEntity entity = mAdapter.getDataList().get(index);
        switch (status) {
            case Resource.SUCCESS:
                entity.setCodeStatus(CodeBean.STATUS_CODE_SUCCESS);
    ...
    public void onClick(View view) {
        ...
        if (id == R.id.tv_scan_back) {
            ...
            CommonPackageScanBackActivity.start(this, TYPE_SCAN_BACK, mViewModel.getConfigEntity().getId(),
            ...
            CommonDialogUtil.showSelectDialog(this,"是否退出当前包装关联操作?", "若退出,您所操作的数据将被放入待执行.",
                    "继续退出", "立即上传", new CommonDialog.OnButtonClickListener() {
                        ...
                        public void onLeftClick() {
                            MainActivity.start(PackagingAssociationActivity.this, 0);
                        }
    ...
    private void showUploadFinishDialog(UploadResultBean data) {
        ...
        CommonDialogUtil.showSelectDialog(this,"数据上传成功", details, "返回工作台", "查看状态", new CommonDialog.OnButtonClickListener() {
            ...
            public void onRightClick() {
                MainActivity.start(PackagingAssociationActivity.this, 0);
                TaskListActivity.start(TaskListViewModel.TYPE_TASK_PACKAGING_ASSOCIATION, PackagingAssociationActivity.this);
            }
        });
    }

    private void updateStatisticsCount(int boxCount) {
        updateCodeCountView();
     ...
     * @param boxCount
     */
    private void updateBoxCountView(int boxCount) {
    ...
    public static void start(Context context, long configId) {
        if (isActivityNotFinished(context)) {
            Intent intent = new Intent(context, PackagingAssociationActivity.class);
